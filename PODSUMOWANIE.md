...
- **OTA przez własny WiFi AP + HTTP upload, na żądanie** (2026-09-08, sprawdzone na sprzęcie działa end-to-end): po jednorazowym wgraniu tej wersji przez USB, kolejne aktualizacje idą przez WiFi (zwykły `curl -F "firmware=@firmware.bin" http://192.168.4.1/update`) — bez kabla i bez przełącznika. Celowo NIE ArduinoOTA/espota — ten protokół wymaga połączenia zwrotnego z ESP32 do komputera, co Windows Firewall na sieci publicznej blokuje domyślnie (wymaga uprawnień administratora do naprawy, niewygodne w praktyce, potwierdzone empirycznie). HTTP POST wymaga tylko połączenia wychodzącego z komputera — nigdy nie blokowane. WiFi AP jest **domyślnie wyłączone** (nie startuje przy boocie) — uruchamia je dopiero komenda BLE `ota_enable` (przycisk w PWA, panel Debug), aktywne ~10 min albo do końca trwającego transferu, potem gaśnie samo (albo ręcznie przez `ota_disable`) — minimalizuje czas ekspozycji sieci. SSID/hasło AP mają bezpieczne domyślne wartości w `config.h`, nadpisywalne lokalnie przez `firmware/include/wifi_secrets.h` (**gitignored**, NIE trafia do publicznego repo). BLE/Modbus działają całkowicie niezależnie od WiFi. **Uwaga na zapas pamięci Flash**: 92.2% partycji OTA (~102KB wolnego) — niewiele przestrzeni na dalszy rozrost firmware
